...
class GameQuestionGenerator():
	...
	def point_type_only_variants(self):
		questions = []
		point_type_variants = zip(self.point_type_counter, self.point_type_counter.values())
		point_variants = zip(self.point_counter, self.point_counter.values())

		total_baskets_sentence = QuestionTemplate.total_baskets_format.format(total=self.total_baskets) # {} total baskets were made.
		total_points_sentence = QuestionTemplate.total_points_format.format(total=self.total_points) # {} total points were made.

		for tup in [(point_type_variants, total_baskets_sentence), (point_variants, total_points_sentence)]:
			variants, total_sentence = tup
			for i in range(len(variants)):
				answer_point_type, answer = [variants[j] for j in range(len(variants)) if j == i][0]
				information_in_question = [variants[j] for j in range(len(variants)) if j != i]
				
				templates = []
				for info in information_in_question:
					point_type, point_type_count = info
					point_type = self.get_user_friendly_point_type(point_type=point_type)
					templates.append(QuestionTemplate.point_type_format.format(point_type_count=point_type_count, point_type=point_type))

				question_info_sentence = ". ".join(templates)

				question_prompt = QuestionTemplate.point_type_question_format.format(point_type=self.get_user_friendly_point_type(point_type=answer_point_type))

				combined = "{}. {}. {}".format(total_sentence, question_info_sentence, question_prompt)
				print combined, "({answer})".format(answer=answer)
				question = GameQuestion(text=combined, correct_answer=answer, question_subject=self.question_subject, game_id=self.game_id, game_title=self.game_title, game_location=self.game_location, game_date=self.game_date)
	...
	def team_only_variants(self):
		questions = []
		team_basket_variants = zip(self.team_basket_counter, self.team_basket_counter.values())
		team_variants = zip(self.team_counter, self.team_counter.values())

		total_baskets_sentence = QuestionTemplate.total_baskets_format.format(total=self.total_baskets) # {} total baskets were made.
		total_points_sentence = QuestionTemplate.total_points_format.format(total=self.total_points) # {} total points were made.

		for tup in [(team_basket_variants, total_baskets_sentence, "baskets", QuestionTemplate.point_type_question_format), (team_variants, total_points_sentence, "points", QuestionTemplate.point_type_question_format)]:
			variants, total_sentence, descriptor, current_question_prompt = tup
			for i in range(len(variants)):
				answer_team, answer = [variants[j] for j in range(len(variants)) if j == i][0]
				information_in_question = [variants[j] for j in range(len(variants)) if j != i]

				templates = []
				for info in information_in_question:
					team, team_count = info
					team = self.get_user_friendly_team_point_string(team=team, descriptor=descriptor)
					templates.append(QuestionTemplate.point_type_format.format(point_type_count=team_count, point_type=team))
					
				question_info_sentence = ". ".join(templates)

				question_prompt = current_question_prompt.format(point_type=self.get_user_friendly_team_point_string(team=answer_team, descriptor=descriptor))
	...
	def quarter_only_variants(self):
		questions = []
		quarter_basket_variants = zip(self.quarter_baskets_counter, self.quarter_baskets_counter.values())
		quarter_variants = zip(self.quarter_counter, self.quarter_counter.values())

		total_baskets_sentence = QuestionTemplate.total_baskets_format.format(total=self.total_baskets) # {} total baskets were made.
		total_points_sentence = QuestionTemplate.total_points_format.format(total=self.total_points) # {} total points were made.
		for tup in [(quarter_basket_variants, total_baskets_sentence, "baskets", QuestionTemplate.point_type_question_format), (quarter_variants, total_points_sentence, "points", QuestionTemplate.point_type_question_format)]:
			variants, total_sentence, descriptor, question_prompt = tup
			for i in range(len(variants)):
				answer_quarter, answer = [variants[j] for j in range(len(variants)) if j == i][0]
				information_in_question = [variants[j] for j in range(len(variants)) if j != i]
				templates = []
				for info in information_in_question:
					quarter, quarter_count = info
					quarter = self.get_user_friendly_quarter_string(quarter=quarter)
					templates.append(QuestionTemplate.point_type_format.format(point_type_count="{count} {descriptor}".format(count=quarter_count, descriptor=descriptor), point_type=quarter))

				question_info_sentence = ". ".join(templates)

				combined = "{}. {}. {}".format(total_sentence, question_info_sentence, question_prompt.format(point_type="{descriptor} {quarter}".format(descriptor=descriptor, quarter=self.get_user_friendly_quarter_string(quarter=answer_quarter))))
				print combined, "({answer})".format(answer=answer)
				question = GameQuestion(text=combined, correct_answer=answer, question_subject=self.question_subject, game_id=self.game_id, game_title=self.game_title, game_location=self.game_location, game_date=self.game_date)
				questions.append(question)
		return questions
	...
	def player_only_variants(self):
		questions = []
		player_basket_variants = zip(self.player_baskets_counter, self.player_baskets_counter.values())
		player_variants = zip(self.player_counter, self.player_counter.values())

		total_baskets_sentence = QuestionTemplate.teams_total_baskets_format.format(total=self.total_baskets) # {} baskets were made in total by both teams
		total_points_sentence = QuestionTemplate.teams_total_points_format.format(total=self.total_points) # {} points were made in total by both teams
		question_prompt = QuestionTemplate.player_question_format
		
		for tup in [(player_basket_variants, total_baskets_sentence, "baskets"), (player_variants, total_points_sentence, "points")]:
			variants, total_sentence, descriptor = tup
			for i in range(len(variants)):
				player, ct = variants[i]
				player = self.get_user_friendly_player_string(player=player)
				information_in_question = [variants[j] for j in range(len(variants)) if j != i]
				other = sum([tup[1] for tup in information_in_question])
				...
				player_sentence = QuestionTemplate.player_score_format.format(player=player, ct=ct, descriptor=descriptor)
				combined = "{}. {}. {}".format(total_sentence, player_sentence, question_prompt.format(descriptor=descriptor, subject="the rest of the team"))
				print combined, "({answer})".format(answer=other)

				question = GameQuestion(text=combined, correct_answer=other, question_subject=self.question_subject, game_id=self.game_id, game_title=self.game_title, game_location=self.game_location, game_date=self.game_date)
				questions.append(question)
		return questions
		

		# variants = zip(self.point_counter, self.point_counter.values())
		# question_subjects = []
		# for i in range(len(variants)):
		# 	question_subjects.append([variants[j] for j in range(len(variants)) if j != i] + [self.total_points])
		# question_subjects.append(variants)

		# question_template1 = " {} were {} and {} were {}. How many {} were there total?"
		# question_template2 = "{} total baskets were made. {} were {} and {} were {}. How many {} were there total?"
		# question_template3 = 
		# question_template4 = "How many total baskets were made if {} were {}, {} were {}, and {} were {}?"

		# question_template5 = "{} total baskets were made. {} were {} and {} were {}. How many {} were there total?"
		# question_template6 = "{} total baskets were made. {} were {} and {} were {}. How many {} were there total?"
		# question_template7 = "{} total baskets were made. {} were {} and {} were {}. How many {} were there total?"
		# question_template8 = "How many total baskets were made if {} were {}, {} were {}, and {} were {}?"
